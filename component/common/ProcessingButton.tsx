...
const ProcessingButton = (props: Props) => {
    const { color, icon, label, loading, onClicked, type } = props
    const css = color === 'white' 
    ? 'btn btn-default'
    : `bg-${color}-700 border border-transparent rounded-md shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-${color}-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-${color}-500`
    return (
        <button
            disabled={loading}
            type={type}
